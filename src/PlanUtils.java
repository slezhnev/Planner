...
public class PlanUtils extends DefaultHandler {
    ...
    public static boolean savePlanToFile() {
        ...
        if (!res) {
            JOptionPane.showMessageDialog(null, "Ошибка сохранения плана",
                    "Сохранение плана", JOptionPane.ERROR_MESSAGE);
            return false;
        } else {
            ...
            //
            JOptionPane.showMessageDialog(null, "План сохранен в файле " +
                    file.getPath(), "Сохранение плана",
    ...
    private static class Plan {
         ...
         *            Работы
         */
        private Plan(int quarter, String year, List<Worker> workers,
                List<PlanPart> works) {
    ...
    private static boolean savePlanToJSON(File file) {
        ...
        Plan plan =
                new Plan(Starter.getMainForm().getSelectedQuarter(), Starter
                        .getMainForm().getYear(), Starter.getMainForm()
                        .getWorkers(), Starter.getMainForm().getPlan());
        try {
    ...
    private static boolean savePlanToXML(File file) {
        ...
        root.setAttribute("quarter", "" +
                (Starter.getMainForm().getSelectedQuarter()));
        ...
        for (PlanPart part : Starter.getMainForm().getPlan()) {
            ...
            for (WorkInPlan work : part.getWorks()) {
                ...
                for (WorkerInPlan worker : work.getWorkersInPlan()) {
                    ...
                    for (int i = 0; i < 3; i++)
                        e4.setAttribute("perMonth" + (i + 1), "" +
                                worker.getPerMonth()[i]);
                    e3.appendChild(e4);
                }
                //
                e2.appendChild(e3);
        ...
        try {
            transformer.transform(domSource, new StreamResult(new FileWriter(
                    file)));
     ...
     *             В случае ошибок ввода-вывода при формировании и сохранении
     *             XML
     * @throws javax.xml.parsers.ParserConfigurationException
     *             см. @javax.xml.parsers.DocumentBuilderFactory.newInstance().
     *             newDocumentBuilder()
     * @throws javax.xml.transform.TransformerException
     *             см. @javax.xml.transform.Transformer
     */
    public static void makeQuarterPlan(int type) throws IOException,
        ...
        for (PlanPart part : Starter.getMainForm().getPlan()) {
            ...
            for (WorkInPlan work : part.getWorks()) {
                ...
                if (work.getLaborTotal() == 0) {
                    ...
                    sLabor =
                            "" + (work.getLaborTotal() * 22) + " ч/д (" +
                                    work.getLaborTotal() + " ч/м)";
                }
                ...
                for (WorkerInPlan worker : work.getWorkersInPlan()) {
                    for (int i = 0; i < 3; i++)
                        makedLabor = makedLabor + worker.getPerMonth()[i];
                }
                ...
                for (WorkInPlan.WorkStage stage : work.getStages()) {
                    ...
                    e6.appendChild(e7);
                    for (WorkInPlan.WorkInStage workInStage : stage
                            .getWorksInStage()) {
            ...
            if (type == 0) {
                ...
                e2.setAttribute("makedTotal", (part.getWorks().size() != 0)
                        ? ("" + Math.round(100 * makedTotal /
                                part.getWorks().size())) : "-");
                e2.setAttribute("makedLabor", ((laborTotal != 0) ? ("" +
                        Math.round(100 * makedLabor / laborTotal) + "% (" +
                        makedLabor + " / " + laborTotal + " ч/м)") : "-"));
            }
            //
            e1.appendChild(e2);
            //
        ...
        transformer.setOutputProperty(OutputKeys.INDENT, "yes");
        transformer
                .transform(domSource, new StreamResult(new FileWriter(file)));
    }
     ...
     *            Какой диалог отображать. true - загрузки, false - сохранения
    ...
    public static File selectPlanFile(String dialogTitle, boolean showLoadDialog) {
        if (fc == null) {
            ...
            final FileNameExtensionFilter plan2 =
                    new FileNameExtensionFilter("Файлы планов версии 2",
                            "plan2");
            fc.addChoosableFileFilter(plan2);
            fc.addChoosableFileFilter(new FileNameExtensionFilter(
                    "Файлы планов версии 1", "plan"));
    ...
    public static boolean loadPlanFromFile() {
        ...
        if (!res) {
            JOptionPane.showMessageDialog(null, "Ошибка загрузки плана",
                    "загрузка плана", JOptionPane.ERROR_MESSAGE);
            return false;
        } else {
            loadedFrom = file;
            JOptionPane.showMessageDialog(null, "План успешно загружен",
                    "Загрузка плана", JOptionPane.INFORMATION_MESSAGE);
            return true;
        }
    }
    ...
    private void parse(File file) throws SAXException,
    ...
    public void startElement(String uri, String localName, String qName,
        ...
        if ("plan".equalsIgnoreCase(qName)) {
            try {
                Starter.getMainForm().setSelectedQuarter(
                        Integer.parseInt(attributes.getValue("quarter")) - 1);
            ...
            // Если мы строим список работников - то будем добавлять его. Если
            // нет - то будем искать...
            if (inWorkers) {
                Starter.getMainForm().getWorkers().add(
                        new Worker(attributes.getValue("name"), 0.0, Boolean
                                .valueOf(attributes.getValue("isOverhead")),
                                Boolean.valueOf(attributes
                                        .getValue("isNotInReport"))));
            } else if (inWork) {
                if (tempWork != null) {
                    ...
                    for (Worker worker : Starter.getMainForm().getWorkers()) {
                        if (worker.getName().equals(name)) {
                            try {
                                WorkerInPlan workerInPlan =
                                        new WorkerInPlan(worker, Double
                                                .parseDouble(attributes
                                                        .getValue("labor")));
                                workerInPlan.getPerMonth()[0] =
                                        Double.parseDouble(attributes
                                                .getValue("perMonth1"));
                                workerInPlan.getPerMonth()[1] =
                                        Double.parseDouble(attributes
                                                .getValue("perMonth2"));
                                workerInPlan.getPerMonth()[2] =
                                        Double.parseDouble(attributes
                                                .getValue("perMonth3"));
                                tempWork.getWorkersInPlan().add(workerInPlan);
                            } catch (NumberFormatException e) {
                                throw new SAXException(
                                        "Ошибка чтения работника в работе");
                            }
                            break;
                        }
                    }
            ...
            tempPlanPart =
                    new PlanPart(attributes.getValue("name"), attributes
                            .getValue("longName"));
            ...
            if ((attributes.getValue("maked") == null) ||
                    (!attributes.getValue("maked").equals("1"))) {
                tempWork.setMaked(false);
                if (attributes.getValue("makedPercent") == null) {
                    tempWork.setMakedPercent(0.0);
                } else {
                    try {
                        tempWork.setMakedPercent(Double.parseDouble(attributes
                                .getValue("makedPercent")));
     ...
     *             см. @javax.xml.transform.Transformer
     */
    public static void makeMonthPlan(int month, String monthName)
            ...
            if (!worker.isNotInMonthReport()) {
                ...
                if (worker.isOverhead()) {
                    ...
                    for (int i = 0; i < Starter.getMainForm().getPlan().size(); i++) {
                        for (WorkInPlan work : Starter.getMainForm().getPlan()
                                .get(i).getWorks()) {
                            for (WorkerInPlan workerInPlan : work
                                    .getWorkersInPlan()) {
                                if (worker.getName().equals(
                                        workerInPlan.getWorker().getName())) {
                                    // Значит это тот работник
                                    totalLaborInMonth =
                                            totalLaborInMonth +
                                                    workerInPlan.getPerMonth()[month];
        ...
        for (PlanPart planPart : Starter.getMainForm().getPlan()) {
            for (WorkInPlan work : planPart.getWorks()) {
                ...
                for (WorkerInPlan worker : work.getWorkersInPlan()) {
                    totalLaborInMonth =
                            totalLaborInMonth + worker.getPerMonth()[month];
                }
                if (works.containsKey(work.getName())) {
                    works.put(work.getName(), works.get(work.getName()) +
                            totalLaborInMonth);
                } else {
     ...
     *             см. @javax.xml.transform.Transformer
     */
    public static void makePerWorkerPlan() throws IOException,
            TransformerException, ParserConfigurationException {
        ...
        for (Worker worker : Starter.getMainForm().getWorkers()) {
            ...
            if (worker.isOverhead()) {
                ...
                double totalLabor = 0;
                for (int i = 0; i < Starter.getMainForm().getPlan().size(); i++) {
                    for (WorkInPlan work : Starter.getMainForm().getPlan().get(
                            i).getWorks()) {
                        for (WorkerInPlan workerInPlan : work
                                .getWorkersInPlan()) {
                            if (worker.getName().equals(
                                ...
                                totalLabor =
                                        totalLabor +
                                                workerInPlan.getLaborContent();
                                if (workerInPlan.getLaborContent() > 0) {
                                    if (works.containsKey(work.getName())) {
                                        works.put(work.getName(), works
                                                .get(work.getName()) +
                                                workerInPlan.getLaborContent());
                                    } else {
                                        works.put(work.getName(), workerInPlan
                                                .getLaborContent());
                                    }
                                }
                            }
