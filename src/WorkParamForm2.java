 ...
 */
public class WorkParamForm2 extends JDialog implements ActionListener, ItemListener, ListSelectionListener {
     ...
     * Отображает диалог
     *
     * @param frameComp  Главный frame приложения
     * @param workToEdit Работа для редактирования
    ...
    private WorkParamForm2(Frame frame,
                           WorkInPlan workToEdit) {
        ...
        stagesTable.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        stagesTable.setModel(new StageTableModel(new ArrayList<WorkInPlan.WorkStage>()));
        //
        worksInStageTable.setDefaultRenderer(String.class, new MultilineTableCell());
        worksInStageTable.getSelectionModel().setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
        worksInStageTable.setModel(new WorkInStageTableModel(new ArrayList<WorkInPlan.WorkInStage>()));
        //
        if (workToEdit != null) {
            ...
            nameWorkEdit.setText("");
            makedCheckBox.setSelected(false);
    ...
    public void actionPerformed(ActionEvent e) {
        switch (e.getActionCommand()) {
            case "Сохранить": {
                ...
                res.setWorkType((PlanUtils.WorkTypes) workTypeCB.getSelectedItem());
                setVisible(false);
                break;
            }
            case "Отмена": {
            ...
            case "Добавить": {
                ...
                ((StageTableModel) stagesTable.getModel()).fireTableDataChanged();
                stagesTable.getSelectionModel().setSelectionInterval(res.getStages().size() - 1, res.getStages().size() - 1);
            ...
            case "Изменить": {
                ...
                if ((row > -1) && (row < res.getStages().size())) {
                    String stageName = WorkStageParamDialog.showDialog(stagesTable,
                            res.getStages().get(row).getName());
                    if (stageName != null) {
            ...
            case "Удалить": {
                int row = stagesTable.getSelectedRow();
                if ((row > -1) && (row < res.getStages().size())) {
                    if (JOptionPane.showConfirmDialog(worksInStageTable, "Вы уверены, " +
            ...
            case "^": {
                ...
                if ((row > 0) && (res.getStages().size() > 1)) {
                    ...
                    ((StageTableModel) stagesTable.getModel()).fireTableDataChanged();
                    stagesTable.getSelectionModel().setSelectionInterval(row - 1, row - 1);
                }
                break;
            }
            case "\\/": {
    ...
    public void valueChanged(ListSelectionEvent e) {
        if (!e.getValueIsAdjusting()) {
            ...
            if (selectedRow < res.getStages().size()) {
                worksInStageTable.setModel(new WorkInStageTableModel(res.getStages().get(selectedRow).getWorksInStage()));
                ((WorkInStageTableModel) worksInStageTable.getModel()).fireTableDataChanged();
            }
        }
    }
...
// DO NOT EDIT OR ADD ANY CODE HERE!
    ...
    private void $$$setupUI$$$() {
        ...
        panel8.setBorder(BorderFactory.createTitledBorder(BorderFactory.createLineBorder(new Color(-16777216)), "Работы этапа"));
        ...
        panel11.add(workTypeCB, new GridConstraints(0, 0, 1, 1, GridConstraints.ANCHOR_WEST, GridConstraints.FILL_HORIZONTAL, GridConstraints.SIZEPOLICY_CAN_GROW, GridConstraints.SIZEPOLICY_FIXED, null, null, null, 0, false));
    }
        ...
        public void actionPerformed(ActionEvent e) {
            switch (e.getActionCommand()) {
                case "Добавить": {
                    ...
                    ((WorkInStageTableModel) worksInStageTable.getModel()).fireTableDataChanged();
                    worksInStageTable.getSelectionModel().setSelectionInterval(works.size() - 1, works.size() - 1);
                ...
                case "Изменить": {
                    ...
                    if ((row > -1) && (row < works.size())) {
                        WorkInPlan.WorkInStage work = WorkInStageParamForm.showDialog(worksInStageTable,
                                works.get(row));
                        if (work != null) {
                            works.set(row, work);
                            ((WorkInStageTableModel) worksInStageTable.getModel()).fireTableDataChanged();
                ...
                case "Удалить": {
                    ...
                    if ((row > -1) && (row < works.size())) {
                        if (JOptionPane.showConfirmDialog(worksInStageTable, "Вы уверены, " +
                                "что хотите удалить выбранную работу в этапе?", "Удаление работы",
                                JOptionPane.YES_NO_OPTION) == JOptionPane.OK_OPTION) {
                            works.remove(row);
                            ((WorkInStageTableModel) worksInStageTable.getModel()).fireTableDataChanged();
                        }
                ...
                case "^": {
                    ...
                    if ((row > 0) && (res.getStages().size() > 1)) {
                        ...
                        works.set(row, work);
                        ((WorkInStageTableModel) worksInStageTable.getModel()).fireTableDataChanged();
                        worksInStageTable.getSelectionModel().setSelectionInterval(row - 1, row - 1);
                    }
                    break;
                }
                case "\\/": {
    ...
    /**
     * Multiline table cell rendered
     * Сперто с http://stackoverflow.com/questions/965023/how-to-wrap-lines-in-a-jtable-cell
     */
